...
ALTER TABLE order_fulfillments ADD COLUMN logistics_synced_at INTEGER NOT NULL DEFAULT 0;
...
CREATE TABLE fulfillment_ship_operations (id TEXT PRIMARY KEY,user_id INTEGER NOT NULL REFERENCES users(id),order_id TEXT NOT NULL,idempotency_key TEXT NOT NULL,status TEXT NOT NULL,request_json TEXT NOT NULL DEFAULT '',response_json TEXT NOT NULL DEFAULT '',error_message TEXT NOT NULL DEFAULT '',created_at INTEGER NOT NULL,finished_at INTEGER NOT NULL DEFAULT 0,UNIQUE(user_id,idempotency_key));
