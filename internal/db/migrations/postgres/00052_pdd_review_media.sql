-- +goose Up
CREATE TABLE pdd_review_media (id BIGSERIAL PRIMARY KEY,goods_id TEXT NOT NULL,review_id TEXT NOT NULL,sku_id TEXT NOT NULL DEFAULT '',media_key TEXT NOT NULL,media_type TEXT NOT NULL,source_type TEXT NOT NULL DEFAULT 'initial',remote_url TEXT NOT NULL,cover_url TEXT NOT NULL DEFAULT '',media_md5 TEXT NOT NULL DEFAULT '',width INTEGER NOT NULL DEFAULT 0,height INTEGER NOT NULL DEFAULT 0,duration_ms BIGINT NOT NULL DEFAULT 0,is_live_photo_image INTEGER NOT NULL DEFAULT 0,collected_at BIGINT NOT NULL,last_seen_at BIGINT NOT NULL,UNIQUE(goods_id,media_key));
