...
export interface MaterialPriceStrategy { mode:'manual'|'fixed_add'|'percent_add'|'margin';value:number;minimum_profit_cent:number;reprice_mode:'off'|'notify'|'automatic';max_increase_cent:number;max_increase_percent:number;maximum_price_cent:number;cooldown_seconds:number;low_stock_threshold:number }
...
export interface ScheduledPublishCheck { material_id:number;title:string;ok:boolean;minimum_profit_cent:number;enabled_sku_count:number;reasons:string[];material_revision:number }
export interface ScheduledPublishTask { id:string;batch_id:string;material_id:number;title:string;account_id:string;sequence:number;planned_at:number;interval_seconds:number;status:string;attempt_count:number;minimum_profit_cent:number;minimum_actual_profit_cent:number;published_item_id:string;error_stage:string;error_code:string;error_message:string;account_queue_paused:boolean;created_at:number;started_at:number;finished_at:number }
export interface ScheduledPublishBatch { id:string;account_id:string;start_at:number;minimum_profit_cent:number;task_count:number;status:string;created_at:number;finished_at:number;success_count:number;failed_count:number;pending_count:number }
export const preflightScheduledPublish=(input:{material_ids:number[];account_id:string;minimum_profit_cent:number;location?:PublishLocation}):Promise<{items:ScheduledPublishCheck[];minimum_profit_cent:number}>=>post('/scheduled-publish/preflight',input);
export const createScheduledPublishBatch=(input:{material_ids:number[];account_id:string;start_at:number;minimum_profit_cent:number;location?:PublishLocation}):Promise<{id:string;tasks:Array<{id:string;material_id:number;title:string;planned_at:number;interval_seconds:number}>}>=>post('/scheduled-publish/batches',input);
export const getScheduledPublishTasks=(status=''):Promise<ScheduledPublishTask[]>=>get(`/scheduled-publish/tasks${status?`?status=${encodeURIComponent(status)}`:''}`);
export const getScheduledPublishBatches=():Promise<ScheduledPublishBatch[]>=>get('/scheduled-publish/batches');
export const cancelScheduledPublishTask=(id:string)=>post(`/scheduled-publish/tasks/${encodeURIComponent(id)}/cancel`,{});
export const cancelScheduledPublishBatch=(id:string)=>post(`/scheduled-publish/batches/${encodeURIComponent(id)}/cancel`,{});
export const cloneScheduledPublishTask=(id:string,startAt:number):Promise<{success:boolean;batch_id:string;task_id:string;planned_at:number}>=>post(`/scheduled-publish/tasks/${encodeURIComponent(id)}/clone`,{start_at:startAt});
export const resumeScheduledPublishAccount=(accountId:string):Promise<{success:boolean;resumed:number}>=>post(`/scheduled-publish/accounts/${encodeURIComponent(accountId)}/resume`,{});
export const createMaterialFromPDD = (goodsId:string):Promise<{id:number}> => post(`/materials/from-pdd/${encodeURIComponent(goodsId)}`,{});
export const getMaterials = (query = '',publishStatus=''):Promise<ProductMaterial[]> => {const params=new URLSearchParams();if(query.trim())params.set('q',query.trim());if(publishStatus)params.set('publish_status',publishStatus);const suffix=params.toString();return get(`/materials${suffix?`?${suffix}`:''}`)};
...
export const updateMaterialSKUSource = (materialID:number,materialSKUID:string,input:{action:'bind'|'convert_manual'|'convert_placeholder';source_goods_id?:string;source_sku_id?:string}):Promise<{success:boolean;sku:ProductMaterialSKU}> => put(`/materials/${materialID}/skus/${encodeURIComponent(materialSKUID)}/source`,input);
export interface MaterialSplitGroup { name:string;title?:string;material_sku_ids:string[] }
export interface MaterialSplitResult { success:boolean;batch_id:string;children:Array<{id:number;name:string;title:string;sku_count:number}>;selected_count:number;remaining_count:number }
export const splitMaterial = (materialID:number,groups:MaterialSplitGroup[],expectedVersion:string,idempotencyKey:string):Promise<MaterialSplitResult> => post(`/materials/${materialID}/split`,{groups,expected_version:expectedVersion,idempotency_key:idempotencyKey});
export interface MaterialSourceAnomaly { material_id:number;title:string;material_sku_id?:string;sku_type?:string;source_goods_id?:string;source_sku_id?:string;issue:string }
export const getMaterialSourceAnomalies = ():Promise<{count:number;items:MaterialSourceAnomaly[]}> => get('/materials/source-anomalies');
