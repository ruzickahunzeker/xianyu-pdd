...
import {
  ...
  getPDDProduct, getPDDReviewMedia, PDDReviewMedia, ProductMaterialVideo, publishMaterial, syncMaterialSource, updateMaterial, uploadMaterialImage,
	...
	splitMaterial,
	preflightScheduledPublish,createScheduledPublishBatch,ScheduledPublishCheck,getScheduledPublishTasks,ScheduledPublishTask,cancelScheduledPublishTask,
	getScheduledPublishBatches,ScheduledPublishBatch,cancelScheduledPublishBatch,cloneScheduledPublishTask,resumeScheduledPublishAccount,
} from '../services/api';
...
export type CommonSpecificationText = { specificationName: string; text: string; values: string[] };
...
const trimCommonTextBoundary = (value: string) => value.replace(/^[\s【】\[\]（）()，,、/\\·:：;；+_-]+|[\s【】\[\]（）()，,、/\\·:：;；+_-]+$/g, '');
export const findCommonSpecificationTexts = (specifications: Specification[]): CommonSpecificationText[] => specifications.flatMap(specification => {
  const values = Array.from(new Set(specification.values.map(item => item.value.trim()).filter(Boolean)));
  if (values.length < 2) return [];
  const [shortest] = [...values].sort((left, right) => left.length - right.length);
  const candidates = new Set<string>();
  for (let start = 0; start < shortest.length; start++) for (let end = start + 2; end <= shortest.length; end++) {
    const fragments = trimCommonTextBoundary(shortest.slice(start, end)).split(/[【】\[\]（）()]/).map(trimCommonTextBoundary);
    for (const text of fragments) if (text.length >= 2 && values.every(value => value.includes(text))) candidates.add(text);
  }
  const maximal = [...candidates]
    .filter(text => ![...candidates].some(other => other.length > text.length && other.includes(text)))
    .sort((left, right) => values[0].indexOf(left) - values[0].indexOf(right) || right.length - left.length);
  return maximal.map(text => ({ specificationName: specification.name, text, values }));
});
export const removeCommonSpecificationText = (specifications: Specification[], specificationName: string, text: string) => specifications.map(specification => specification.name !== specificationName ? specification : ({
  ...specification,
  values: specification.values.map(value => ({ ...value, value: value.value.split(text).join('').replace(/\s{2,}/g, ' ').trim() })),
}));
...
function ProductEditor({ initial, mode, accounts, onClose, onSaved }: {
  ...
  const [draft, setDraft] = useState(() => {
	...
	value.price_strategy ||= {mode:'manual',value:0,minimum_profit_cent:50,reprice_mode:'off',max_increase_cent:0,max_increase_percent:0,maximum_price_cent:0,cooldown_seconds:21600,low_stock_threshold:10};
	value.price_strategy.reprice_mode ||= 'off';
	value.price_strategy.max_increase_cent ||= 0;
	value.price_strategy.max_increase_percent ||= 0;
	value.price_strategy.maximum_price_cent ||= 0;
	value.price_strategy.cooldown_seconds ||= 21600;
	value.price_strategy.low_stock_threshold ||= 10;
  ...
  const [batchStock, setBatchStock] = useState('');
	const [batchStockDimension, setBatchStockDimension] = useState('');
	const [batchStockValues, setBatchStockValues] = useState<string[]>([]);
	const [batchStockIncludeDisabled, setBatchStockIncludeDisabled] = useState(false);
	...
	const [productImageChoices, setProductImageChoices] = useState<Array<{url:string;goods_id:string;source:'pdd_gallery'|'pdd_detail'|'legacy'}>>([]);
	...
	const [mediaPreview, setMediaPreview] = useState<null|{key?:string;type:'image'|'video';url:string;cover?:string;scope?:'picker'|'material'}>(null);
  ...
  const [commonTextOpen, setCommonTextOpen] = useState(false);
  const [commonTextUndo, setCommonTextUndo] = useState<Specification[] | null>(null);
  ...
  const dimensions = useMemo(() => specifications.map(item => item.name), [specifications]);
	const batchStockOptions = useMemo(() => specifications.find(item => item.name === batchStockDimension)?.values.map(item => item.value).filter(Boolean) || [], [specifications, batchStockDimension]);
	const commonSpecificationTexts = useMemo(() => findCommonSpecificationTexts(specifications), [specifications]);
	const batchStockMatches = useMemo(() => draft.skus.filter(sku => {
		if (sku.sku_type === 'placeholder' || (!batchStockIncludeDisabled && sku.enabled === false)) return false;
		if (!batchStockDimension) return true;
		if (!batchStockValues.length) return false;
		return sku.properties.some(property => property.name === batchStockDimension && batchStockValues.includes(property.value));
	}), [draft.skus, batchStockDimension, batchStockValues, batchStockIncludeDisabled]);
	...
	useEffect(() => {
		...
		void Promise.all(sourceGoodsIDs.map(async goodsID => {
			...
			setProductImageChoices(Array.from(new Map(rows.flatMap(row => {
				const gallery=(row.product.gallery_images||[]).map(url=>({url,goods_id:row.goodsID,source:'pdd_gallery' as const}));
				const galleryURLs=new Set(gallery.map(item=>item.url));
				const detail=(row.product.detail_images||[]).filter(url=>!galleryURLs.has(url)).map(url=>({url,goods_id:row.goodsID,source:'pdd_detail' as const}));
				const known=new Set([...gallery,...detail].map(item=>item.url));
				const legacy=row.product.images.filter(url=>!known.has(url)).map(url=>({url,goods_id:row.goodsID,source:'legacy' as const}));
				return [...gallery,...detail,...legacy].map(item=>[item.url,item] as const);
			})).values()));
	...
	const toggleMediaSelection = (key:string) => {
		...
		const selected = new Set(mediaPicker.selected);
		if (selected.has(key)) selected.delete(key);
		else {
			const limit=mediaPicker.kind==='review-video'?Number.POSITIVE_INFINITY:Math.max(0,9-draft.images.length);
			if(selected.size>=limit){setError(`商品图片最多 9 张，当前还可加入 ${limit} 张`);return;}
			selected.add(key);
		}
		setMediaPicker({...mediaPicker,selected});
	};
	const moveMediaPreview = (direction:-1|1) => {
		if (mediaPreview?.scope==='material') {
			const index=draft.images.indexOf(mediaPreview.url);
			if(index<0||draft.images.length<2)return;
			const url=draft.images[(index+direction+draft.images.length)%draft.images.length];
			setMediaPreview({key:url,type:'image',url,scope:'material'});
			return;
		}
		...
		setMediaPreview({key:next.key,type:next.type,url:next.url,cover:'cover' in next?next.cover:'',scope:'picker'});
	};
	const operateMaterialImage=(url:string,action:'main'|'up'|'down'|'delete')=>{
		const index=draft.images.indexOf(url);if(index<0)return;
		if(action==='delete'&&draft.images.length<=1){setError('素材至少需要保留 1 张图片');return;}
		if(action==='delete'&&draft.skus.some(sku=>sku.properties.some(property=>property.image_url===url))&&!confirm('该图片正在被规格使用；删除后对应规格图片也会清空，是否继续？'))return;
		const images=[...draft.images];
		if(action==='main'){images.splice(index,1);images.unshift(url);}
		...
		if(action==='delete')images.splice(index,1);
		setDraft(current=>({...current,images,image_metadata:current.image_metadata.filter(item=>images.includes(item.url)),skus:action==='delete'?current.skus.map(sku=>({...sku,properties:sku.properties.map(property=>property.image_url===url?{...property,image_url:undefined}:property)})):current.skus}));
		if(action==='delete'){const next=images[Math.min(index,images.length-1)];setMediaPreview(next?{key:next,type:'image',url:next,scope:'material'}:null);}
		else setMediaPreview({key:url,type:'image',url,scope:'material'});
	};
	useEffect(() => {
		...
		const onKeyDown = (event:KeyboardEvent) => {
			...
			else if(mediaPreview.scope==='material'&&event.key.toLowerCase()==='m'){event.preventDefault();operateMaterialImage(mediaPreview.url,'main');}
			else if(mediaPreview.scope==='material'&&event.key==='Delete'){event.preventDefault();operateMaterialImage(mediaPreview.url,'delete');}
			else if(mediaPreview.scope==='material'&&event.shiftKey&&event.key==='ArrowLeft'){event.preventDefault();operateMaterialImage(mediaPreview.url,'up');}
			else if(mediaPreview.scope==='material'&&event.shiftKey&&event.key==='ArrowRight'){event.preventDefault();operateMaterialImage(mediaPreview.url,'down');}
	...
	const confirmMediaPicker = () => {
		...
		if(mediaPicker.kind!=='review-video'&&selected.filter(item=>!draft.images.includes(item.url)).length>9-draft.images.length){setError('所选图片加入后会超过 9 张，请减少选择');return;}
		if (mediaPicker.kind === 'review-video') {
			...
			setDraft(current=>{
				...
				const metadata=Array.from(new Map([...current.image_metadata,...images.filter(url=>selectedURLs.has(url)).map(url=>{const item=selectedByURL.get(url)!;return {url,source:mediaPicker.kind==='product-image'?item.sourceType:'pdd_review',source_goods_id:item.goodsID,status:'valid'};})].map(item=>[item.url,item])).values()).filter(item=>images.includes(item.url));
				return {...current,images,image_metadata:metadata};
			});
		}
		setMediaPicker(null);
	};
	const previewItems=mediaPreview?.scope==='material'?draft.images.map(url=>({key:url,url,type:'image' as const})):pickerChoices;
	const previewIndex=mediaPreview?previewItems.findIndex(item=>item.key===mediaPreview.key):-1;
  ...
  const applySpecifications = (next: Specification[]) => {
	...
	const deleteCommonSpecificationText = (candidate: CommonSpecificationText) => {
		const next = removeCommonSpecificationText(specifications, candidate.specificationName, candidate.text);
		const changed = next.find(item => item.name === candidate.specificationName);
		if (!changed || changed.values.some(item => !item.value.trim())) {
			setError(`不能删除“${candidate.text}”：删除后存在空规格值`);
			return;
		}
		const normalizedValues = changed.values.map(item => item.value.trim());
		if (new Set(normalizedValues).size !== normalizedValues.length) {
			setError(`不能删除“${candidate.text}”：删除后“${candidate.specificationName}”出现重复规格值`);
			return;
		}
		const previewSKUs = renameSKUProperties(draft.skus, specifications, next);
		const keys = previewSKUs.map(sku => skuKey(sku.properties));
		if (new Set(keys).size !== keys.length) {
			setError(`不能删除“${candidate.text}”：删除后会产生重复 SKU 组合`);
			return;
		}
		setCommonTextUndo(clone(specifications));
  ...
  const patchSKU = (index: number, patch: Partial<ProductMaterialSKU>) => {
	...
	const adjustAllPrices = () => {
		...
		if (batchStockDimension && !batchStockValues.length) return setError('请至少选择一个规格值');
		if (!batchStockMatches.length) return setError('当前筛选条件没有可调价的 SKU');
		if (batchStockMatches.some(sku => sku.price_cent + adjustment <= 0)) return setError('调价后售价必须大于 0');
		const matched = new Set(batchStockMatches);
		setDraft(current => ({ ...current, skus: current.skus.map(sku => matched.has(sku) ? { ...sku, price_cent: sku.price_cent + adjustment } : sku) }));
		setError('');
	};
	const setFilteredStock = () => {
		const quantity = Number(batchStock);
		if (!Number.isInteger(quantity) || quantity < 0) return setError('库存必须是大于等于 0 的整数');
		if (draft.stock_strategy.mode !== 'manual') return setError('批量设置库存仅适用于“手工库存”，请先切换库存策略');
		if (batchStockDimension && !batchStockValues.length) return setError('请至少选择一个规格值');
		if (!batchStockMatches.length) return setError('当前筛选条件没有可修改的 SKU');
		if (quantity === 0 && !window.confirm(`将把 ${batchStockMatches.length} 个 SKU 的库存设为 0，是否继续？`)) return;
		const matched = new Set(batchStockMatches);
		setDraft(current => ({ ...current, skus: current.skus.map(sku => sku.sku_type === 'placeholder' ? { ...sku, quantity: 0 } : matched.has(sku) ? { ...sku, quantity } : sku) }));
		setError('');
	};
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-100">
    ...
    <main className="mx-auto grid max-w-7xl gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="space-y-5">
		...
		{draft.source_type === 'pdd' && <section className="rounded-2xl border bg-white p-5 shadow-sm space-y-4">
		  <div><h3 className="font-black">价格与低库存联动</h3><p className="mt-1 text-xs text-slate-500">仅在扩展采集、人工更新或采购核验取得可信数据后判断；不会定时访问拼多多。</p></div>
		  <div className="grid gap-4 sm:grid-cols-2">
		    <label className="text-sm font-bold">拼多多涨价处理<select className="mt-2 w-full rounded-xl border p-3 font-normal" value={draft.price_strategy.reprice_mode} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,reprice_mode:event.target.value as typeof draft.price_strategy.reprice_mode}})}><option value="off">关闭</option><option value="notify">只提醒</option><option value="automatic">自动修改闲鱼售价</option></select></label>
		    <label className="text-sm font-bold">低库存阈值<input type="number" min="1" max="100" className="mt-2 w-full rounded-xl border p-3 font-normal" value={draft.price_strategy.low_stock_threshold} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,low_stock_threshold:Math.max(1,Number(event.target.value)||10)}})}/><span className="mt-1 block text-xs font-normal text-slate-400">精确库存低于该值时向下同步，默认10</span></label>
		    <label className="text-sm font-bold">单次最大加价（元，0不限）<input type="number" min="0" step="0.01" className="mt-2 w-full rounded-xl border p-3 font-normal" value={draft.price_strategy.max_increase_cent?money(draft.price_strategy.max_increase_cent):''} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,max_increase_cent:Math.max(0,Math.round((Number(event.target.value)||0)*100))}})}/></label>
		    <label className="text-sm font-bold">单次最大涨幅（%，0不限）<input type="number" min="0" step="0.1" className="mt-2 w-full rounded-xl border p-3 font-normal" value={draft.price_strategy.max_increase_percent||''} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,max_increase_percent:Math.max(0,Number(event.target.value)||0)}})}/></label>
		    <label className="text-sm font-bold">最高闲鱼售价（元，0不限）<input type="number" min="0" step="0.01" className="mt-2 w-full rounded-xl border p-3 font-normal" value={draft.price_strategy.maximum_price_cent?money(draft.price_strategy.maximum_price_cent):''} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,maximum_price_cent:Math.max(0,Math.round((Number(event.target.value)||0)*100))}})}/></label>
		    <label className="text-sm font-bold">同商品冷却时间（小时）<input type="number" min="1" className="mt-2 w-full rounded-xl border p-3 font-normal" value={Math.max(1,Math.round(draft.price_strategy.cooldown_seconds/3600))} onChange={event=>setDraft({...draft,price_strategy:{...draft.price_strategy,cooldown_seconds:Math.max(1,Number(event.target.value)||6)*3600}})}/></label>
		  </div>
		  {draft.price_strategy.reprice_mode==='automatic'&&<p className="rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-800">自动执行仍受最低利润、SKU稳定映射、最大涨幅、账号状态和回读确认约束；任何异常都会阻塞，不会循环重试。</p>}
		</section>}
	        <section className="rounded-2xl border bg-white p-5 shadow-sm"><div className="mb-4 flex flex-wrap items-center justify-between gap-2"><h3 className="font-black">商品规格</h3><div className="flex items-center gap-2"><button type="button" className="rounded-lg border px-3 py-1.5 text-xs font-bold" onClick={()=>setCommonTextOpen(true)}>检查公共文字</button>{commonTextUndo&&<button type="button" className="rounded-lg border px-3 py-1.5 text-xs font-bold text-indigo-600" onClick={()=>{applySpecifications(commonTextUndo);setCommonTextUndo(null);setError('');}}>撤销文字删除</button>}<span className="text-xs text-slate-400">最多添加 2 个规格类型</span></div></div>
		  ...
		  <div className="mb-4 space-y-3 rounded-xl border bg-slate-50 p-3"><div><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold">批量操作范围</span><select className="rounded-lg border bg-white p-2 text-sm" value={batchStockDimension} onChange={event=>{setBatchStockDimension(event.target.value);setBatchStockValues([]);}}><option value="">全部 SKU</option>{dimensions.filter(Boolean).map(name=><option key={name} value={name}>按{name}筛选</option>)}</select><label className="flex items-center gap-1 text-xs"><input type="checkbox" checked={batchStockIncludeDisabled} onChange={event=>setBatchStockIncludeDisabled(event.target.checked)}/>包含已停用 SKU</label><span className="text-xs font-bold text-brand">命中 {batchStockMatches.length} 个</span></div>{batchStockDimension&&<div className="mt-2 flex flex-wrap gap-2">{batchStockOptions.map(value=><label key={value} className={`flex cursor-pointer items-center gap-1 rounded-lg border px-2 py-1 text-xs ${batchStockValues.includes(value)?'border-brand bg-brand/10 text-brand':'bg-white'}`}><input type="checkbox" checked={batchStockValues.includes(value)} onChange={event=>setBatchStockValues(current=>event.target.checked?[...current,value]:current.filter(item=>item!==value))}/>{value}</label>)}</div>}</div><div className="flex flex-wrap items-center gap-2 border-t pt-3"><span className="text-sm font-bold">统一调价</span><input type="number" step="0.01" className="w-28 rounded-lg border bg-white p-2" placeholder="如 5 或 -5" value={priceAdjustment} onChange={event => setPriceAdjustment(event.target.value)}/><button type="button" className="rounded-lg bg-brand px-3 py-2 text-sm font-bold text-white" onClick={adjustAllPrices}>应用到 {batchStockMatches.length} 个</button><span className="text-xs text-slate-500">正数加价，负数降价</span></div><div className="flex flex-wrap items-center gap-2 border-t pt-3"><span className="text-sm font-bold">统一库存</span><input type="number" min="0" step="1" className="w-28 rounded-lg border bg-white p-2" placeholder="如 100" value={batchStock} onChange={event => setBatchStock(event.target.value)}/><button type="button" className="rounded-lg border bg-white px-3 py-2 text-sm font-bold" onClick={setFilteredStock}>应用到 {batchStockMatches.length} 个</button><span className="text-xs text-slate-500">仅手工库存策略可用；占位 SKU 始终为 0</span></div><p className="text-xs text-slate-500">调价只修改闲鱼售价，不修改拼多多采购价。预计毛利 = 闲鱼售价 − 拼多多采购价（暂未扣除平台费用和运费）。</p></div>
      ...
      <aside className="space-y-5">
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="mb-4 flex justify-between"><h3 className="font-black">商品图片</h3><span className="text-xs text-slate-400">{draft.images.length}/9</span></div>
          <div className="grid grid-cols-3 gap-2">{draft.images.map((url,index)=><div className="group relative aspect-square" key={`${url}-${index}`}><img src={url} referrerPolicy="no-referrer" className="h-full w-full rounded-xl object-cover"/>{index===0&&<span className="absolute bottom-1 left-1 rounded bg-brand px-1.5 py-0.5 text-[10px] text-white">主图</span>}<div className="absolute inset-x-1 top-1 hidden flex-wrap gap-1 group-hover:flex"><button className="rounded bg-brand px-1.5 py-1 text-[10px] text-white" onClick={()=>operateMaterialImage(url,'main')}>设为主图</button><button className="rounded bg-black/60 p-1 text-white" onClick={()=>setMediaPreview({key:url,type:'image',url,scope:'material'})}>预览</button><button disabled={index===0} className="rounded bg-black/60 p-1 text-white disabled:opacity-30" onClick={()=>operateMaterialImage(url,'up')}><ArrowUp className="h-3 w-3"/></button><button disabled={index===draft.images.length-1} className="rounded bg-black/60 p-1 text-white disabled:opacity-30" onClick={()=>operateMaterialImage(url,'down')}><ArrowDown className="h-3 w-3"/></button><button disabled={draft.images.length<=1} className="rounded bg-red-500 p-1 text-white disabled:opacity-30" onClick={()=>operateMaterialImage(url,'delete')}><Trash2 className="h-3 w-3"/></button></div></div>)}</div>
          <div className="mt-3 flex flex-wrap gap-2">{productImageChoices.length>0&&<button className="rounded-lg border px-3 py-2 text-xs font-bold" onClick={()=>setMediaPicker({kind:'product-image',selected:new Set()})}>商品采集图 {productImageChoices.length}</button>}{reviewImageChoices.length>0&&<button className="rounded-lg border px-3 py-2 text-xs font-bold" onClick={()=>setMediaPicker({kind:'review-image',selected:new Set()})}>评论图片 {reviewImageChoices.length}</button>}{draft.images.length<9&&<label className="cursor-pointer rounded-lg border px-3 py-2 text-xs font-bold">本地上传<input type="file" accept="image/*" multiple className="hidden" onChange={async event=>{const files=Array.from(event.target.files||[]).slice(0,9-draft.images.length);const urls=await Promise.all(files.map(async file=>(await uploadMaterialImage(file)).url));setDraft(current=>({...current,images:[...current.images,...urls],image_metadata:[...current.image_metadata,...urls.map(url=>({url,source:'upload',status:'valid'}))]}));}}/></label>}</div>
        </section>
	    ...
	    {mediaPicker&&<div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-6"><div className="max-h-[85vh] w-full max-w-5xl overflow-auto rounded-2xl bg-white p-5"><div className="flex items-center justify-between"><h3 className="text-lg font-black">{mediaPicker.kind==='product-image'?'选择商品采集图':mediaPicker.kind==='review-image'?'选择评论图片':'选择评论视频'}</h3><button onClick={()=>setMediaPicker(null)}><X/></button></div><div className="mt-4 flex flex-wrap gap-2">{pickerGoodsIDs.length>1&&<select className="rounded-lg border px-3 py-2 text-sm" value={mediaGoodsFilter} onChange={event=>{setMediaGoodsFilter(event.target.value);setMediaSKUFilter('');}}><option value="">全部商品</option>{pickerGoodsIDs.map(id=><option key={id}>{id}</option>)}</select>}{mediaPicker.kind==='product-image'?<select className="rounded-lg border px-3 py-2 text-sm" value={mediaSourceFilter} onChange={event=>setMediaSourceFilter(event.target.value)}><option value="">全部采集图片</option><option value="pdd_gallery">商品主图</option><option value="pdd_detail">商品详情图</option><option value="legacy">历史图片</option></select>:<><select className="rounded-lg border px-3 py-2 text-sm" value={mediaSKUFilter} onChange={event=>setMediaSKUFilter(event.target.value)}><option value="">全部 SKU</option>{pickerSKUIds.map(id=><option key={id}>{id}</option>)}</select><select className="rounded-lg border px-3 py-2 text-sm" value={mediaSourceFilter} onChange={event=>setMediaSourceFilter(event.target.value)}><option value="">普通评价和追评</option><option value="initial">普通评价</option><option value="additional">追评</option></select></>}<span className="self-center text-xs text-slate-500">显示 {pickerChoices.length} / {unfilteredPickerChoices.length}，已选 {mediaPicker.selected.size}{mediaPicker.kind!=='review-video'?`，还可加入 ${Math.max(0,9-draft.images.length)} 张`:''}</span></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{pickerChoices.map(item=>{const included=item.type==='image'&&draft.images.includes(item.url);return <article key={item.key} className={`rounded-xl border-2 p-2 ${mediaPicker.selected.has(item.key)?'border-brand':'border-slate-200'}`}><button className="block w-full" onClick={()=>setMediaPreview({key:item.key,type:item.type,url:item.url,cover:'cover' in item?item.cover:'',scope:'picker'})}>{item.type==='video'?<video src={item.url} poster={'cover' in item?item.cover:''} preload="metadata" className="aspect-square w-full rounded-lg bg-black object-contain"/>:<img src={item.url} referrerPolicy="no-referrer" className="aspect-square w-full rounded-lg object-cover"/>}</button><button disabled={included} className="mt-2 w-full rounded-lg border px-2 py-1 text-xs font-bold disabled:bg-slate-100 disabled:text-slate-400" onClick={()=>toggleMediaSelection(item.key)}>{included?'已加入素材':mediaPicker.selected.has(item.key)?'已选择':'选择'}</button><p className="mt-1 truncate text-[10px] text-slate-500">{item.label}</p></article>})}</div><div className="sticky bottom-0 mt-4 flex justify-end gap-2 bg-white py-2"><button className="rounded-lg border px-4 py-2" onClick={()=>setMediaPicker(null)}>取消</button><button className="rounded-lg bg-brand px-4 py-2 font-bold text-white" onClick={confirmMediaPicker}>加入所选媒体（{mediaPicker.selected.size}）</button></div></div></div>}
	    {commonTextOpen&&<div role="dialog" aria-modal="true" aria-label="检查规格公共文字" className="fixed inset-0 z-[65] flex items-center justify-center bg-black/60 p-6"><div className="max-h-[85vh] w-full max-w-3xl overflow-auto rounded-2xl bg-white p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="text-lg font-black">检查规格公共文字</h3><p className="mt-1 text-xs text-slate-500">只列出同一规格类型的每个规格值都包含的连续文字。系统不会自动删除；每项由您单独决定。</p></div><button type="button" onClick={()=>setCommonTextOpen(false)}><X/></button></div>{commonSpecificationTexts.length===0?<div className="mt-5 rounded-xl bg-slate-50 p-5 text-sm text-slate-500">没有检测到长度至少为 2、且每个规格值都包含的公共文字。</div>:<div className="mt-5 space-y-4">{commonSpecificationTexts.map(candidate=><article key={`${candidate.specificationName}-${candidate.text}`} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className="mr-2 rounded bg-slate-100 px-2 py-1 text-xs font-bold">{candidate.specificationName}</span><b>“{candidate.text}”</b><span className="ml-2 text-xs text-slate-400">包含于全部 {candidate.values.length} 个规格值</span></div><button type="button" className="rounded-lg border border-red-200 px-3 py-2 text-sm font-bold text-red-600" onClick={()=>deleteCommonSpecificationText(candidate)}>删除这一项</button></div><div className="mt-3 space-y-1 text-xs">{candidate.values.slice(0,4).map(value=><div key={value} className="grid gap-1 rounded bg-slate-50 p-2 sm:grid-cols-2"><span className="truncate text-slate-500">{value}</span><span className="truncate font-bold">→ {value.split(candidate.text).join('').replace(/\s{2,}/g,' ').trim()}</span></div>)}{candidate.values.length>4&&<p className="text-slate-400">另有 {candidate.values.length-4} 个规格值同样处理</p>}</div></article>)}</div>}<div className="mt-5 flex justify-end"><button type="button" className="rounded-lg bg-slate-900 px-4 py-2 font-bold text-white" onClick={()=>setCommonTextOpen(false)}>完成</button></div></div></div>}
	    {mediaPreview&&<div role="dialog" aria-modal="true" aria-label="媒体预览" className="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 p-6" onClick={()=>setMediaPreview(null)}>{mediaPreview.key&&previewItems.length>1&&<><button type="button" aria-label="上一项" className="absolute left-4 z-10 rounded-full bg-white/15 p-3 text-white hover:bg-white/25" onClick={event=>{event.stopPropagation();moveMediaPreview(-1);}}><ArrowLeft/></button><button type="button" aria-label="下一项" className="absolute right-4 z-10 rounded-full bg-white/15 p-3 text-white hover:bg-white/25" onClick={event=>{event.stopPropagation();moveMediaPreview(1);}}><ArrowRight/></button></>}{mediaPreview.type==='video'?<video key={mediaPreview.url} src={mediaPreview.url} poster={mediaPreview.cover} controls autoPlay className="max-h-[82vh] max-w-full" onClick={e=>e.stopPropagation()}/>:<img src={mediaPreview.url} referrerPolicy="no-referrer" className="max-h-[82vh] max-w-full object-contain" onClick={e=>e.stopPropagation()}/>}<button aria-label="关闭预览" className="absolute right-6 top-6 text-white" onClick={()=>setMediaPreview(null)}><X/></button>{mediaPreview.key&&<div className="absolute bottom-5 left-1/2 flex max-w-[95vw] -translate-x-1/2 flex-wrap items-center justify-center gap-2 rounded-2xl bg-black/70 px-4 py-2 text-sm text-white" onClick={event=>event.stopPropagation()}><span>{Math.max(0,previewIndex)+1} / {previewItems.length}</span>{mediaPreview.scope==='material'?<><button disabled={previewIndex===0} className="rounded-full bg-white/20 px-3 py-1 font-bold disabled:opacity-40" onClick={()=>operateMaterialImage(mediaPreview.url,'main')}>设为主图</button><button disabled={previewIndex<=0} className="rounded-full bg-white/20 px-3 py-1 disabled:opacity-40" onClick={()=>operateMaterialImage(mediaPreview.url,'up')}>上移</button><button disabled={previewIndex<0||previewIndex>=previewItems.length-1} className="rounded-full bg-white/20 px-3 py-1 disabled:opacity-40" onClick={()=>operateMaterialImage(mediaPreview.url,'down')}>下移</button><button disabled={draft.images.length<=1} className="rounded-full bg-red-500 px-3 py-1 font-bold disabled:opacity-40" onClick={()=>operateMaterialImage(mediaPreview.url,'delete')}>删除</button><span className="hidden text-xs text-white/60 lg:inline">← → 切换 · M 主图 · Shift+←/→ 移动 · Delete 删除 · Esc 关闭</span></>:<><button type="button" className={`rounded-full px-3 py-1 font-bold ${mediaPicker?.selected.has(mediaPreview.key)?'bg-brand':'bg-white/20'}`} onClick={()=>toggleMediaSelection(mediaPreview.key!)}>{mediaPicker?.selected.has(mediaPreview.key)?'已选择':'选择'}</button><span className="hidden text-xs text-white/60 sm:inline">← → 切换 · Enter/空格选择 · Esc 关闭</span></>}</div>}</div>}
  </div>;
}
...
const localDateTimeValue=(date:Date)=>{const offset=date.getTimezoneOffset()*60000;return new Date(date.getTime()-offset).toISOString().slice(0,16)};

function ScheduledPublishDialog({materials,accounts,onClose,onCreated}:{materials:ProductMaterial[];accounts:AccountDetail[];onClose:()=>void;onCreated:()=>Promise<void>}){
  const [accountID,setAccountID]=useState(accounts.find(account=>account.enabled)?.id||'');
  const [startAt,setStartAt]=useState(()=>localDateTimeValue(new Date(Date.now()+10*60*1000)));
  const [location,setLocation]=useState<PublishLocation|null>(null);
  const [checks,setChecks]=useState<ScheduledPublishCheck[]|null>(null);
  const [busy,setBusy]=useState(false);
  const [error,setError]=useState('');
  const materialIDs=materials.map(material=>material.id);
  const runPreflight=async()=>{if(!accountID)return setError('请选择发布账号');setBusy(true);setError('');try{const result=await preflightScheduledPublish({material_ids:materialIDs,account_id:accountID,minimum_profit_cent:30,location:location||undefined});setChecks(result.items);}catch(reason){setError(reason instanceof Error?reason.message:'发布预检失败');}finally{setBusy(false)}};
  const create=async()=>{if(!checks||checks.some(check=>!check.ok))return setError('请先完成预检，并移除未通过的素材');const timestamp=Math.floor(new Date(startAt).getTime()/1000);if(!Number.isFinite(timestamp))return setError('请选择有效开始时间');setBusy(true);setError('');try{const result=await createScheduledPublishBatch({material_ids:materialIDs,account_id:accountID,start_at:timestamp,minimum_profit_cent:30,location:location||undefined});alert(`已创建定时发布批次，共 ${result.tasks.length} 个任务；发布间隔随机5～10分钟`);await onCreated();onClose();}catch(reason){setError(reason instanceof Error?reason.message:'创建定时发布任务失败');}finally{setBusy(false)}};
  return <div className="fixed inset-0 z-50 overflow-auto bg-black/60 p-4 sm:p-8"><div className="mx-auto max-w-4xl space-y-4 rounded-2xl bg-slate-50 p-5"><header className="flex justify-between"><div><h2 className="text-xl font-black">批量定时发布</h2><p className="mt-1 text-sm text-slate-500">已选 {materials.length} 个素材；同一账号依次发布，间隔随机5～10分钟。失败不自动重试。</p></div><button onClick={onClose}><X/></button></header><section className="grid gap-4 rounded-2xl border bg-white p-5 md:grid-cols-2"><label className="text-sm font-bold">发布账号<select className="mt-2 w-full rounded-xl border p-3 font-normal" value={accountID} onChange={event=>{setAccountID(event.target.value);setChecks(null);setLocation(null)}}><option value="">请选择账号</option>{accounts.map(account=><option key={account.id} value={account.id}>{account.nickname||account.remark||account.id}{account.enabled?'':'（未启用）'}</option>)}</select></label><label className="text-sm font-bold">开始时间<input type="datetime-local" className="mt-2 w-full rounded-xl border p-3 font-normal" value={startAt} onChange={event=>{setStartAt(event.target.value);setChecks(null)}}/></label><div className="md:col-span-2"><PublishLocationPicker accountID={accountID} value={location} onChange={value=>{setLocation(value);setChecks(null)}}/></div><div className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800 md:col-span-2">发布安全线：所有启用SKU的单件利润必须严格大于 ¥0.30；执行前会再次核验价格、库存和SKU映射。每个任务仅执行一次。</div></section><section className="rounded-2xl border bg-white p-5"><h3 className="font-black">素材与预检结果</h3><div className="mt-3 max-h-80 space-y-2 overflow-auto">{materials.map(material=>{const check=checks?.find(item=>item.material_id===material.id);return <div key={material.id} className="rounded-xl border p-3"><div className="flex items-center justify-between gap-3"><b className="truncate">{material.title}</b><span className={`shrink-0 text-xs font-bold ${!check?'text-slate-400':check.ok?'text-emerald-600':'text-red-600'}`}>{!check?'待预检':check.ok?`通过 · 最低利润 ¥${money(check.minimum_profit_cent)}`:'未通过'}</span></div>{check&&!check.ok&&<ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-red-600">{check.reasons.map((reason,index)=><li key={index}>{reason}</li>)}</ul>}</div>})}</div></section>{error&&<p className="rounded-xl bg-red-50 p-3 text-sm font-bold text-red-600">{error}</p>}<footer className="flex justify-end gap-3"><button className="rounded-xl border bg-white px-5 py-2" onClick={onClose}>取消</button><button disabled={busy} className="rounded-xl border bg-white px-5 py-2 font-bold text-brand disabled:opacity-50" onClick={()=>void runPreflight()}>{busy?'检查中…':'发布预检'}</button><button disabled={busy||!checks||checks.some(check=>!check.ok)} className="rounded-xl bg-brand px-5 py-2 font-bold text-white disabled:opacity-40" onClick={()=>void create()}>创建定时任务</button></footer></div></div>;
}

function ScheduledPublishWorkbench({accounts,onChanged}:{accounts:AccountDetail[];onChanged:()=>Promise<void>}){
  const [tasks,setTasks]=useState<ScheduledPublishTask[]>([]);const [batches,setBatches]=useState<ScheduledPublishBatch[]>([]);const [open,setOpen]=useState(false);const [status,setStatus]=useState('');
  const load=async()=>{const [nextTasks,nextBatches]=await Promise.all([getScheduledPublishTasks(status),getScheduledPublishBatches()]);setTasks(nextTasks);setBatches(nextBatches)};useEffect(()=>{if(open)void load()},[open,status]);
  const accountName=(id:string)=>{const account=accounts.find(item=>item.id===id);return account?.nickname||account?.remark||id};
  const labels:Record<string,string>={pending:'待执行',publishing:'发布中',success:'成功',failed:'失败',blocked:'账号暂停',cancelled:'已取消',running:'执行中',paused:'已暂停',completed:'已完成',completed_with_failures:'部分失败'};
  return <section className="rounded-2xl border bg-white"><button className="flex w-full items-center justify-between p-4 text-left" onClick={()=>setOpen(!open)}><div><h3 className="font-black">定时发布工作台</h3><p className="text-xs text-slate-500">查看批次进度、失败原因和账号暂停任务</p></div><span>{open?'收起':'展开'}</span></button>{open&&<div className="space-y-4 border-t p-4"><div><div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-black">最近批次</h4><button className="rounded-lg border px-3 py-1.5 text-xs font-bold" onClick={()=>void load()}>刷新</button></div><div className="grid gap-2 md:grid-cols-2">{batches.slice(0,6).map(batch=><article key={batch.id} className="rounded-xl border p-3 text-xs"><div className="flex justify-between gap-2"><b>{accountName(batch.account_id)}</b><span className="font-bold">{labels[batch.status]||batch.status}</span></div><p className="mt-1 text-slate-500">{batch.success_count}/{batch.task_count} 成功 · {batch.failed_count} 失败 · {batch.pending_count} 待执行</p><p className="mt-1 text-slate-400">开始 {new Date(batch.start_at*1000).toLocaleString()}</p><div className="mt-2 flex gap-4">{batch.status==='paused'&&<button className="font-bold text-emerald-600" onClick={async()=>{if(confirm('确认账号状态已恢复？这会恢复该账号所有尚未执行的定时任务。')){await resumeScheduledPublishAccount(batch.account_id);await load();await onChanged()}}}>恢复账号队列</button>}{['pending','running','paused'].includes(batch.status)&&<button className="font-bold text-red-500" onClick={async()=>{if(confirm('取消该批次所有未执行任务？')){await cancelScheduledPublishBatch(batch.id);await load();await onChanged()}}}>取消剩余任务</button>}</div></article>)}</div></div><div><div className="mb-3 flex flex-wrap gap-2"><select className="rounded-lg border px-3 py-2 text-sm" value={status} onChange={event=>setStatus(event.target.value)}><option value="">全部任务</option><option value="pending">待执行</option><option value="publishing">发布中</option><option value="success">成功</option><option value="failed">失败</option><option value="blocked">账号暂停</option><option value="cancelled">已取消</option></select></div><div className="max-h-96 space-y-2 overflow-auto">{tasks.length===0?<p className="py-6 text-center text-sm text-slate-400">暂无任务</p>:tasks.map(task=><article key={task.id} className="rounded-xl border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><b>{task.title}</b><span className="font-bold">{labels[task.status]||task.status}</span></div><p className="mt-1 text-xs text-slate-500">{accountName(task.account_id)} · 计划 {new Date(task.planned_at*1000).toLocaleString()} · 间隔 {Math.round(task.interval_seconds/60)} 分钟</p>{task.minimum_actual_profit_cent!==0&&<p className="mt-1 text-xs">实际最低利润：¥{money(task.minimum_actual_profit_cent)}</p>}{task.published_item_id&&<p className="mt-1 text-xs">闲鱼商品：{task.published_item_id}</p>}{task.error_message&&<p className="mt-2 rounded bg-red-50 p-2 text-xs text-red-600">{task.error_message}</p>}<div className="mt-2 flex gap-4">{['pending','blocked'].includes(task.status)&&<button className="text-xs font-bold text-red-500" onClick={async()=>{if(confirm('取消该定时发布任务？')){await cancelScheduledPublishTask(task.id);await load();await onChanged()}}}>取消任务</button>}{['failed','cancelled'].includes(task.status)&&<button className="text-xs font-bold text-brand" onClick={async()=>{if(confirm('按当前素材和账号复制为一个新任务，并在1分钟后执行？系统会重新预检，不会重试原任务。')){await cloneScheduledPublishTask(task.id,Math.floor(Date.now()/1000)+60);await load();await onChanged()}}}>复制为新任务</button>}</div></article>)}</div></div></div>}</section>;
}
...
const ProductMaterials: React.FC = () => {
  ...
  const load = async () => {const rows=await getMaterials(activeSearch,publishStatus);setItems(rows);setSelected(current=>new Set([...current].filter(id=>rows.some(row=>row.id===id))))};
  ...
  return <div className="space-y-4"><div><h2 className="text-2xl font-black">发布素材库</h2><p className="text-sm text-slate-500">素材和发布使用同一个商品编辑器；发布时选择账号，不会修改采集源数据。</p></div>
    <div className="flex flex-wrap items-center gap-2"><form className="flex min-w-0 max-w-xl flex-1 gap-2" onSubmit={async event => { event.preventDefault(); const query=search.trim(); setActiveSearch(query); setItems(await getMaterials(query,publishStatus)); }}><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input className="w-full rounded-xl border bg-white py-2.5 pl-10 pr-3" placeholder="搜索素材标题或拼多多商品 ID" value={search} onChange={event=>setSearch(event.target.value)}/></div><button className="rounded-xl bg-brand px-5 font-bold text-white">搜索</button>{activeSearch&&<button type="button" className="rounded-xl border bg-white px-4 font-bold" onClick={async()=>{setSearch('');setActiveSearch('');setItems(await getMaterials('',publishStatus));}}>清除</button>}</form><select className="rounded-xl border bg-white px-3 py-2.5 text-sm" value={publishStatus} onChange={async event=>{const value=event.target.value;setPublishStatus(value);setItems(await getMaterials(activeSearch,value));setSelected(new Set())}}><option value="">全部发布状态</option><option value="unpublished">从未发布</option><option value="published">已发布</option><option value="scheduled">已安排</option><option value="failed">发布失败</option></select></div>
    <ScheduledPublishWorkbench accounts={accounts} onChanged={load}/>
    {items.length>0&&<div className="sticky top-16 z-10 flex flex-wrap items-center gap-3 rounded-xl border bg-white/95 p-3 shadow-sm"><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={items.length>0&&items.every(item=>selected.has(item.id))} onChange={event=>setSelected(event.target.checked?new Set(items.map(item=>item.id)):new Set())}/>选择当前列表</label><span className="text-sm text-slate-500">已选 {selected.size} 个</span>{selected.size>0&&<><button className="rounded-lg bg-brand px-4 py-2 text-sm font-bold text-white" onClick={()=>setScheduleOpen(true)}>批量定时发布</button><button className="text-sm font-bold text-slate-500" onClick={()=>setSelected(new Set())}>清空选择</button></>}</div>}
    {items.length === 0 ? <div className="rounded-2xl border bg-white p-10 text-center text-slate-500">{activeSearch?'没有匹配的素材':'暂无素材，请先从采集商品创建素材'}</div> : <div className="grid gap-4 lg:grid-cols-2">{items.map(material => { const sourcePrices = material.skus.map(item=>item.source_price_cent||0).filter(price=>price>0); const salePrices = material.skus.map(item=>item.price_cent); return <article key={material.id} className={`relative rounded-2xl border bg-white p-5 shadow-sm ${selected.has(material.id)?'ring-2 ring-brand':''}`}><label className="absolute right-4 top-4 z-10 rounded-lg bg-white/90 p-2 shadow"><input type="checkbox" checked={selected.has(material.id)} onChange={event=>setSelected(current=>{const next=new Set(current);event.target.checked?next.add(material.id):next.delete(material.id);return next})}/></label><div className="flex gap-4 pr-8">{material.images[0] ? <img src={material.images[0]} referrerPolicy="no-referrer" className="h-24 w-24 rounded-xl object-cover"/> : <div className="h-24 w-24 rounded-xl bg-slate-100"/>}<div className="min-w-0 flex-1"><h3 className="truncate font-black">{material.title}</h3><p className="mt-1 text-xs text-slate-400">来源：{material.source_type === 'pdd' ? '拼多多' : '手工'} · {material.skus.length} SKU · {material.images.length} 图</p>{(material.parent_material_id||0)>0&&<p className="mt-1 text-xs font-bold text-indigo-600">拆分组：{material.split_group_name}</p>}{material.is_split_source&&<p className="mt-1 text-xs font-bold text-amber-600">已生成拆分子素材；原素材仍可用于发布测试</p>}{material.source_type==='pdd'&&<p className="mt-1 text-xs text-slate-500">拼多多商品 ID：<span className="font-mono">{material.source_id}</span></p>}<p className="mt-2 text-sm text-slate-600">闲鱼 ¥{money(Math.min(...salePrices))}{Math.min(...salePrices)!==Math.max(...salePrices)&&`–${money(Math.max(...salePrices))}`}</p>{sourcePrices.length>0&&<p className="text-xs text-slate-400">拼多多 ¥{money(Math.min(...sourcePrices))}{Math.min(...sourcePrices)!==Math.max(...sourcePrices)&&`–${money(Math.max(...sourcePrices))}`}</p>}{(material.published_accounts||[]).length>0&&<p className="mt-2 text-xs text-emerald-600">已发布：{material.published_accounts!.map(item=>`${accounts.find(account=>account.id===item.account_id)?.nickname||accounts.find(account=>account.id===item.account_id)?.remark||item.account_id}（${item.count}次）`).join('、')}</p>}{(material.scheduled_accounts||[]).length>0&&<p className="mt-1 text-xs text-indigo-600">待发布：{material.scheduled_accounts!.map(item=>`${accounts.find(account=>account.id===item.account_id)?.nickname||accounts.find(account=>account.id===item.account_id)?.remark||item.account_id}（${new Date(item.planned_at*1000).toLocaleString()}）`).join('、')}</p>}{(material.scheduled_failure_count||0)>0&&<p className="mt-1 text-xs font-bold text-red-500">定时发布失败记录：{material.scheduled_failure_count}</p>}</div></div><div className="mt-4 flex flex-wrap gap-3"><button onClick={() => setEditor({ material, mode: 'edit' })} className="flex items-center gap-1 font-bold text-brand"><PackagePlus className="h-4 w-4"/>编辑</button><button onClick={() => setEditor({ material, mode: 'publish' })} className="flex items-center gap-1 font-bold text-emerald-600"><Send className="h-4 w-4"/>发布</button>{!material.parent_material_id&&<button onClick={()=>setSplitTarget(material)} className="flex items-center gap-1 font-bold text-indigo-600"><Scissors className="h-4 w-4"/>拆分</button>}<button onClick={() => setEditor({ material: { ...clone(material), id: material.id }, mode: 'edit' })} className="hidden items-center gap-1 font-bold text-slate-500"><Copy className="h-4 w-4"/>复制</button><button onClick={async () => { if (confirm('删除此素材？不会影响采集商品和闲鱼平台商品。')) { await deleteMaterial(material.id); await load(); } }} className="ml-auto flex items-center gap-1 font-bold text-red-500"><Trash2 className="h-4 w-4"/>删除</button></div></article>; })}</div>}
    {editor && <ProductEditor initial={editor.material} mode={editor.mode} accounts={accounts} onClose={() => setEditor(null)} onSaved={load}/>} {splitTarget&&<MaterialSplitDialog material={splitTarget} onClose={()=>setSplitTarget(null)} onSaved={load}/>} {scheduleOpen&&<ScheduledPublishDialog materials={items.filter(item=>selected.has(item.id))} accounts={accounts} onClose={()=>setScheduleOpen(false)} onCreated={async()=>{setSelected(new Set());await load()}}/>}</div>;
};
