    ...
    <main className="mx-auto grid max-w-7xl gap-5 p-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="space-y-5">
		...
		{draft.source_type === 'pdd' && <section className="rounded-2xl border bg-white p-5 shadow-sm"><div><h3 className="font-black">组合采集商品</h3><p className="mt-1 text-xs text-slate-500">输入已采集的拼多多商品 ID，将它的 SKU 合并到当前素材；每个 SKU 保留独立的 goods_id + sku_id 映射。</p></div><div className="mt-4 flex gap-2"><input className="min-w-0 flex-1 rounded-xl border p-3" placeholder="拼多多 goods_id" value={combineGoodsID} onChange={event=>setCombineGoodsID(event.target.value)}/><button type="button" disabled={combining} className="rounded-xl bg-brand px-4 font-bold text-white disabled:opacity-50" onClick={()=>void combinePDDProduct()}>{combining?'读取中…':'合并商品'}</button></div><div className="mt-3 flex flex-wrap gap-2">{sourceGoodsIDs.map(goodsID=><span key={goodsID} className="flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-xs"><span className="font-mono">{goodsID}</span>{sourceGoodsIDs.length>1&&<button type="button" title="从素材移除此来源及其 SKU" className="text-red-500" onClick={()=>removePDDSource(goodsID)}><X className="h-3 w-3"/></button>}</span>)}</div></section>}
		  ...
		  <div className="mb-4 space-y-3 rounded-xl border bg-slate-50 p-3"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold">闲鱼统一调价</span><input type="number" step="0.01" className="w-28 rounded-lg border bg-white p-2" placeholder="如 5 或 -5" value={priceAdjustment} onChange={event => setPriceAdjustment(event.target.value)}/><button type="button" className="rounded-lg bg-brand px-3 py-2 text-sm font-bold text-white" onClick={adjustAllPrices}>应用</button></div><div className="border-t pt-3"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-bold">批量库存</span><select className="rounded-lg border bg-white p-2 text-sm" value={batchStockDimension} onChange={event=>{setBatchStockDimension(event.target.value);setBatchStockValues([]);}}><option value="">全部 SKU</option>{dimensions.filter(Boolean).map(name=><option key={name} value={name}>按{name}筛选</option>)}</select><input type="number" min="0" step="1" className="w-28 rounded-lg border bg-white p-2" placeholder="如 100" value={batchStock} onChange={event => setBatchStock(event.target.value)}/><button type="button" className="rounded-lg border bg-white px-3 py-2 text-sm font-bold" onClick={setFilteredStock}>应用到 {batchStockMatches.length} 个</button><label className="flex items-center gap-1 text-xs"><input type="checkbox" checked={batchStockIncludeDisabled} onChange={event=>setBatchStockIncludeDisabled(event.target.checked)}/>包含已停用 SKU</label></div>{batchStockDimension&&<div className="mt-2 flex flex-wrap gap-2">{batchStockOptions.map(value=><label key={value} className={`flex cursor-pointer items-center gap-1 rounded-lg border px-2 py-1 text-xs ${batchStockValues.includes(value)?'border-brand bg-brand/10 text-brand':'bg-white'}`}><input type="checkbox" checked={batchStockValues.includes(value)} onChange={event=>setBatchStockValues(current=>event.target.checked?[...current,value]:current.filter(item=>item!==value))}/>{value}</label>)}</div>}<p className="mt-2 text-xs text-slate-500">当前命中 {batchStockMatches.length} 个 SKU；占位 SKU 永远保持库存 0。批量修改仅在“手工库存”策略下可用。</p></div><p className="text-xs text-slate-500">调价只修改闲鱼售价，不修改拼多多采购价。预计毛利 = 闲鱼售价 − 拼多多采购价（暂未扣除平台费用和运费）。</p></div>
