...
import { FolderOpen, Laptop, PackageSearch } from 'lucide-react';
...
const PDDCollector: React.FC = () => {
  const [tab, setTab] = useState<'products' | 'materials' | 'devices'>('products');
  return <div className="space-y-6">
    ...
    <div className="flex w-fit gap-1 rounded-xl bg-slate-100 p-1">
      ...
      <button onClick={() => setTab('materials')} className={`flex items-center gap-2 rounded-lg px-4 py-2 font-bold ${tab === 'materials' ? 'bg-white text-brand shadow-sm' : 'text-slate-500'}`}><FolderOpen className="h-4 w-4" />素材库</button>
    ...
    {tab === 'products' ? <PDDCollectedProducts /> : tab === 'materials' ? <ProductMaterials /> : <PDDCollectorDevices compact />}
  </div>;
};
