        ...
        <div className="space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="ios-card rounded-xl p-6 bg-white space-y-5">
              ...
              <label className="flex items-center justify-between gap-4 rounded-xl bg-gray-50 p-4"><span><span className="block text-sm font-bold">采购时临时修改手机号</span><span className="text-xs text-gray-500">默认关闭。开启后，从闲鱼发货成功起满 1 小时进入恢复手机号与提醒队列。</span></span><input type="checkbox" checked={String(settings.pdd_phone_change_enabled).toLowerCase()==='true'} onChange={e=>setSettings({...settings,pdd_phone_change_enabled:e.target.checked})} className="w-5 h-5"/></label>
            </div>
          </section>
